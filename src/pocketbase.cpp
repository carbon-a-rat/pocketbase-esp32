...
PocketbaseArduino::PocketbaseArduino(const char *baseUrl)
  ...
  for (size_t i = 0; i < sizeof(subscription_ctx) / sizeof(subscription_ctx[0]);
       i++) {
...
bool PocketbaseArduino::update(String collection, String record_id,
                               String &body) {
  String endpoint =
      base_url + String("collections/" + collection + "/records/" + record_id);
...
String PocketbaseArduino::getOne(const char *recordId,
  ...
  String fullEndpoint =
      base_url + String(current_endpoint) + "records/" + recordId;
...
String PocketbaseArduino::getList(const char *page /* = nullptr */,
...
String PocketbaseArduino::create(const String &requestBody) {
  ...
  // Call performPOSTRequest with the constructed endpoint and provided
  // parameters
  return main_connection.performPOSTRequest(fullEndpoint.c_str(), requestBody);
}

void PocketbaseArduino::subscribe(const char *collection, const char *recordid,
                                  SubscriptionFn callback, void *ctx) {
  ...
  Serial.println("Subscribing to collection: " + String(collection) +
  ...
  subscription_ctx[selected_index].tcp_connection =
      new HTTPClient(); // Initialize the HTTPClient for this subscription
  ...
  if (https->begin(*current.pb_connection.client, url)) {
    ...
    https->setReuse(true); // Enable connection reuse for subscription GET
                           //       https->setTimeout(10000); // Set a timeout
                           //       for the connection
    int code = retry_GET(*https); // Retry the GET request with a maximum of 5
                                  // attempts and a delay of 1000 ms
    if (code > 0) {
      Serial.printf("[HTTPS] GET... code: %d\n", code);
      if (code == HTTP_CODE_OK || code == HTTP_CODE_NO_CONTENT) {
        Serial.println("[HTTPS] Connection established successfully.");
        //  current.pb_connection.client->setTimeout(10000); // Set a timeout
        //  for the client
        current.pb_connection.client->setNoDelay(
            true); // Disable Nagle's algorithm for better performance
        current.endpoint = url;

        auto event = query_subscription_response(
            &current); // Read the initial response from the server
    ...
    //  current.pb_connection.client->flush(); // Ensure the client is ready for
    //  reading
    // current.tcp_connection = std::move(https); // Store the HTTPClient in the
    // subscription context
  } else {
    Serial.printf("[HTTPS] Unable to connect\n");
  }

  Serial.println("Subscription established for collection: " +
                 String(collection) + ", recordid: " + String(recordid));
...
SubscriptionEvent
PocketbaseArduino::query_subscription_response(SubscriptionCtx *sub) {
...
void PocketbaseArduino::update_subscription_status(SubscriptionCtx &current) {
  ...
  if (!https_post->begin(*this->main_connection.client,
                         base_url + "realtime")) {
  ...
  Serial.println("[HTTPS] Subscribing to collection: " + String(collection) +
                 ", recordid: " + String(recordid));
  https_post->addHeader("Content-Type", "application/json");
  https_post->addHeader("Authorization",
                        current.pb_connection.auth_token.c_str());
  ...
  doc["clientId"] =
      current.connection_id; // Use the connection ID from the initial response
  ...
  auto code =
      retry_POST(*https_post, payload); // Retry the POST request with a maximum
                                        // of 5 attempts and a delay of 1000 ms
  if (code > 0) {
    ...
    Serial.println(
        "[HTTPS] Subscription POST failed, no response from server.");
  } else {
  ...
  for (size_t i = 0; i < sizeof(subscription_ctx) / sizeof(subscription_ctx[0]);
    ...
    if (subscription_ctx[i].active) {
      if (subscription_ctx[i].pb_connection.client == nullptr) {
        Serial.println(
            "Subscription client is null, skipping update for collection: " +
            subscription_ctx[i].collection +
            ", recordid: " + subscription_ctx[i].recordid);
        continue;
      }
      ...
      if (!stream->connected()) {
        Serial.println("Subscription client is not connected, attempting to "
                       "reconnect for collection: " +
                       current.collection + ", recordid: " + current.recordid);
        current.tcp_connection->begin(*current.pb_connection.client,
                                      current.endpoint);
        current.tcp_connection->addHeader("Accept", "text/event-stream");
        current.tcp_connection->addHeader("Authorization",
                                          current.pb_connection.auth_token);
        //    current.tcp_connection->addHeader("Connection", "keep-alive");
        current.tcp_connection->setFollowRedirects(
      ...
      if (event.event.isEmpty()) {
        Serial.println("No event received for collection: " +
                       current.collection + ", recordid: " + current.recordid);
        continue;
      }

      Serial.println("Received event: " + event.event +
                     ", data: " + event.data + ", id: " + event.id);

      if (event.event == "PB_CONNECT") {
        Serial.println("Subscription connection established with ID: " +
                       event.id);
      ...
      if (current.callback) {
        ...
        Serial.println(
            "No callback function set for subscription to collection: " +
            current.collection + ", recordid: " + current.recordid);
      }
...
void PocketbaseArduino::unsubscribe(const char *collection,
                                    const char *recordid) {
  for (size_t i = 0; i < sizeof(subscription_ctx) / sizeof(subscription_ctx[0]);
       i++) {
    if (subscription_ctx[i].active &&
      ...
      Serial.println("Unsubscribing from collection: " + String(collection) +
                     ", recordid: " + String(recordid));
      subscription_ctx[i].active = false;
      subscription_ctx[i]
          .pb_connection.client->stop(); // Stop the client connection
      delete subscription_ctx[i]
          .pb_connection.client; // Delete the client to free resources
      delete subscription_ctx[i]
          .tcp_connection; // Delete the HTTPClient to free resources
      subscription_count--;
  ...
  Serial.println("No active subscription found for collection: " +
                 String(collection) + ", recordid: " + String(recordid));
}
